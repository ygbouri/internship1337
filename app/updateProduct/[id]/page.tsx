...
export default function AddProduct({ params }: { params: { id: string } }) {
  ...
  const id_article: string = params.id;
  ...
  useEffect(() => {
    if (id_article) {
      const getProducts = async () => {
        const data = await getProduct(id_article);
  ...
  const mutate = useMutation({
    mutationFn: (data: any) => putarticle(data),
    onSettled: () => {
  ...
  const handleSCategorie = (even: any) => {
    ...
    if (text === "") {
      if (products?.description) {
        ...
        setLengthSDesc(200 - products.description.length);
      } else {
        setText(str);
          ...
          <div className={`flex flex-col gap-2`}>
            ...
            <Input
              ...
              onBlur={(e: any) => {
                reference === ""
                  ? setReference(
                      products?.reference ? products.reference : e.target.value
                    )
            ...
            <div className="w-[50%] max-xl:w-full flex flex-col gap-2">
              ...
              <Input
                ...
                onBlur={(e: any) => {
                  etatProduct === ""
                    ? setEtatProduct(
                        products?.etat ? products.etat : e.target.value
                      )
                    : setEtatProduct(e.target.value);
                }}
              />
            </div>
            <div className="w-[50%] max-xl:w-full flex flex-col gap-2">
                  ...
                  onBlur={(e: any) => {
                    {
                      brandItem === ""
                        ? setBrand(
                            products?.marque ? products.marque : e.target.value
                          )
                        : setName(e.target.value);
                    }
                  }}
            ...
            <div className="w-[50%] flex flex-col gap-2 ">
              ...
              <Input
                ...
                onChange={(e: any) => {
                  sellingPrice === 0
                    ? setSellingPrice(
                        products?.prix ? products.prix : e.target.value
                      )
